...
${issFileEntriesIncludedFilePath} = `
    ...
    Rename-Item -PassThru -NewName { $_ -replace @('\.tmp$', '.iss') }
...
Get-Item -Path ${issFileEntriesIncludedFilePath} | Format-List
...
& ${ISCC_PATH} `
    ...
    "/DMY_SOURCE_DIR_PATH=${DEPLOY_DIR_PATH}" `
    "/DMY_FILES_SECTION_FILE_PATH=${issFileEntriesIncludedFilePath}" `
    "/DMY_APP_NAME=${TARGET}" `
    "/DMY_COMPRESSION=${ISS_COMPRESSION}" `
